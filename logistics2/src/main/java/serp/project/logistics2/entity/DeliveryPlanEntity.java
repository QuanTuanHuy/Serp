...
public class DeliveryPlanEntity {
    ...
    public void addSlips(List<DeliverySlipEntity> slips) {
        ...
        this.slips.addAll(slips);
        this.totalSlips = this.slips.size();
    }

    public void removeSlips(List<DeliverySlipEntity> slips) {
        slips.forEach(this.slips::remove);
        this.totalSlips = this.slips.size();
    }

    public void addVehicleShippers(List<VehicleShipperEntity> vehicleShippers) {
        if (PlanOptimizationStatus.valueOf(this.optimizationStatus)
                .ordinal() >= PlanOptimizationStatus.OPTIMIZING.ordinal()) {
            throw new AppException(AppErrorCode.PLAN_IN_OPTIMIZATION);
        }

        this.vehicleShippers.addAll(vehicleShippers);
        this.totalVehicles = this.vehicleShippers.size();
    }

    public void removeVehicleShippers(List<VehicleShipperEntity> vehicleShippers) {
        vehicleShippers.forEach(this.vehicleShippers::remove);
        this.totalVehicles = this.vehicleShippers.size();
    }
