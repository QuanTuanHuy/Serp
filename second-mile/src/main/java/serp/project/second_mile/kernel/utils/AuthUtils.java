...
import serp.project.second_mile.kernel.config.InternalApiAuthenticationDetails;
...
public class AuthUtils {
    ...
    public Optional<String> getCurrentAuditPrincipal() {
        Optional<String> uid = getCurrentUserId().map(String::valueOf);
        if (uid.isPresent()) {
            return uid;
        }
    ...
    public boolean canAccessOrganization(Long organizationId) {
        ...
        if (!hasAccess) {
            log.warn("User tenantId {} does not match organizationId {}", currentTenantId.get(), organizationId);
        }
        return hasAccess;
    }
}
