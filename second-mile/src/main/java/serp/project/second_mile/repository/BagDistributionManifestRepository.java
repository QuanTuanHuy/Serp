...
@Repository
public interface BagDistributionManifestRepository extends JpaRepository<BagDistributionManifest, Long>,
        JpaSpecificationExecutor<BagDistributionManifest> {
    boolean existsByTenantIdAndManifestCodeIgnoreCase(Long tenantId, String manifestCode);

    Optional<BagDistributionManifest> findByIdAndTenantId(Long id, Long tenantId);

    @Query("""
            select count(manifest) > 0
            from BagDistributionManifest manifest
