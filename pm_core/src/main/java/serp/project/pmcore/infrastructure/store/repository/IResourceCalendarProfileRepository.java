...
@Repository
public interface IResourceCalendarProfileRepository extends JpaRepository<ResourceCalendarProfileModel, Long> {
    List<ResourceCalendarProfileModel> findByTenantIdOrderByNameAsc(Long tenantId);

    Optional<ResourceCalendarProfileModel> findByIdAndTenantId(Long id, Long tenantId);

    @Modifying
    @Query("""
            UPDATE ResourceCalendarProfileModel p
