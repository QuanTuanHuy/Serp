...
import serp.project.pmcore.domain.port.client.ICachePort;
...
public class RedisCacheAdapter implements ICachePort {
    ...
    public void put(String namespace, String key, Object value, Duration ttl) {
        ...
        if (ttl == null) {
            redisTemplate.opsForValue().set(cacheKey, jsonValue, resolveDefaultCacheTtl());
            return;
        }
    ...
    public <T> Optional<T> get(String namespace, String key, Class<T> type) {
        ...
        T value = jsonUtils.fromJson(jsonValue, type);
    ...
    public void addToSet(String namespace, String key, String... members) {
        ...
        String cacheKey = keyStrategy.cacheKey(namespace, key);
        redisTemplate.opsForSet().add(cacheKey, members);
    }
    ...
    public List<String> getListRange(String namespace, String key, long start, long end) {
        String cacheKey = keyStrategy.cacheKey(namespace, key);
        List<String> values = redisTemplate.opsForList().range(cacheKey, start, end);
    ...
    public boolean hashExists(String namespace, String key, String field) {
        String cacheKey = keyStrategy.cacheKey(namespace, key);
        return redisTemplate.opsForHash().hasKey(cacheKey, field);
    }

    @Override
    public long hashIncrement(String namespace, String key, String field, long delta) {
        String cacheKey = keyStrategy.cacheKey(namespace, key);
        return redisTemplate.opsForHash().increment(cacheKey, field, delta);
    }
