...
| Field | Value |
|-------|-------|
| **Use Case ID** | UC-PM-044 |
| **Use Case Name** | List Project Roles |
| **Module** | PM Core |
| **Version** | 1.0 |
| **Last Updated** | 2026-04-12 |
| **Priority** | Medium |
...
Return a list of project roles visible to the current tenant. The list includes tenant-owned roles and visible system-owned roles exposed as reusable read-only catalog data. The API never returns soft-deleted rows or rows belonging to another tenant.

### Actors

| Actor | Type | Description |
|-------|------|-------------|
| PM Admin | Primary | Lists project roles in tenant administration scope |
...
2. Response returns paginated tenant-visible project roles
...
|------|-------------|--------|
| 1 | PM Admin | Sends `GET /api/v1/roles` with optional query filters |
...
| 9 | System | Returns HTTP 200 with paginated project-role list |

### Alternative Flows

#### AF-1: List Only Tenant-Owned Roles
...
| Step | Actor/System | Action |
|------|-------------|--------|
| 6.1 | System | Filters result to tenant-owned roles only |

**Rejoins**: Main Flow Step 7

#### AF-2: List Only System-Owned Roles
...
| Step | Actor/System | Action |
|------|-------------|--------|
| 6.1 | System | Filters result to visible system-owned roles only |
...
|---------|-------------|-------------|
| BR-PM-044-01 | List results include tenant-owned roles plus visible system-owned read-only roles | Query layer |
| BR-PM-044-02 | Rows from another tenant must never be returned | Query layer |
| BR-PM-044-03 | Soft-deleted rows are excluded from list results | Query layer |
| BR-PM-044-04 | Default pagination is `page=0`, `pageSize=10`, maximum `pageSize=100` | Controller layer |
| BR-PM-044-05 | Default sort is `name ASC` | Query layer |
| BR-PM-044-06 | Search filter applies case-insensitively to `name` | Query layer |
...
|-------|------|-------------|
| data | ProjectRole[] | Array of visible project roles |
| data[].id | int64 | Role ID |
