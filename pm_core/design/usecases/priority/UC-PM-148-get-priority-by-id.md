...
| Field | Value |
|-------|-------|
| **Use Case ID** | UC-PM-148 |
| **Use Case Name** | Get Priority by ID |
| **Module** | PM Core |
| **Version** | 1.0 |
| **Last Updated** | 2026-04-12 |
| **Priority** | Low |
...
Retrieve one priority visible to the current tenant by ID. Read APIs may return either a tenant-owned priority or a system-owned priority exposed to that tenant as reusable read-only catalog data. Soft-deleted rows and rows from other tenants are never returned.

### Actors

| Actor | Type | Description |
|-------|------|-------------|
| PM Admin | Primary | Reads priority details in tenant administration scope |
...
4. Target priority is visible to the tenant: either `tenant_id=tenantId` or system-owned read-only data
...
2. Response returns the requested visible priority
...
|------|-------------|--------|
| 1 | PM Admin | Sends `GET /api/v1/priorities/{priorityId}` |
| 2 | System | Validates JWT and extracts `tenantId` |
| 3 | System | Validates caller has tenant-scoped PM Admin authority |
| 4 | System | Loads priority by `id=priorityId` if it belongs to the tenant or is a visible system-owned row, and validates `deleted_at IS NULL` |
| 5 | System | Builds response including ownership/read-only metadata |
| 6 | System | Returns HTTP 200 with priority details |

### Alternative Flows

#### AF-1: Read System-Owned Priority
...
|---------|-------------|-------------|
| BR-PM-148-01 | Read lookup is limited to rows visible to the tenant: tenant-owned rows and system-owned rows exposed as reusable catalog data | Repository layer |
| BR-PM-148-02 | Rows from another tenant must not be returned | Repository layer |
| BR-PM-148-03 | Soft-deleted rows are excluded from read lookup | Repository layer |
| BR-PM-148-04 | Returning a system-owned priority through read API does not permit write operations on that row | Service layer |
| BR-PM-148-05 | Read operation is side-effect free and must not update audit fields | Query layer |
