...
func (r *RedisAdapter) DeleteKeyFromRedis(ctx context.Context, key string) error {
	if err := r.redisClient.Del(ctx, key).Err(); err != nil {
		r.logger.Error("Failed to delete key from redis "+key, zap.Error(err))
		return err
	}
	return nil
}
...
func (r *RedisAdapter) ExistsInRedis(ctx context.Context, key string) (bool, error) {
	...
	if err != nil {
		r.logger.Error("Failed to check existence of key in redis "+key, zap.Error(err))
		return false, err
	}
	return exists > 0, nil
}
...
func (r *RedisAdapter) GetFromRedis(ctx context.Context, key string, destination any) error {
	...
	if err != nil {
		...
		r.logger.Error("Failed to get from redis key "+key, zap.Error(err))
		return err
	}
	if err = json.Unmarshal([]byte(data), destination); err != nil {
		r.logger.Error("Failed to unmarshal data for key "+key, zap.Error(err))
		return err
	}
	return nil
}
...
func (r *RedisAdapter) GetHSetFromRedis(ctx context.Context, key string) (map[string]string, error) {
	...
	if err != nil {
		...
		r.logger.Error("Failed to get HSet from redis key "+key, zap.Error(err))
		return nil, err
	}
	return data, nil
}
...
func (r *RedisAdapter) GetKeysFromRedis(ctx context.Context, pattern string) ([]string, error) {
	...
	if err != nil {
		r.logger.Error("Failed to get keys from redis with pattern "+pattern, zap.Error(err))
		return nil, err
	}
	return keys, nil
}
...
func (r *RedisAdapter) SetHSetToRedis(ctx context.Context, key string, mapData map[string]any, ttl int) error {
	if err := r.redisClient.HSet(ctx, key, mapData).Err(); err != nil {
		r.logger.Error("Failed to set HSet to redis key "+key, zap.Error(err))
		return err
	}
	if ttl > 0 {
		if err := r.redisClient.Expire(ctx, key, time.Duration(ttl)*time.Second).Err(); err != nil {
			r.logger.Error("Failed to set TTL for HSet key "+key, zap.Error(err))
			return err
		}
	}
	return nil
...
func (r *RedisAdapter) SetToRedis(ctx context.Context, key string, value any, ttl int) error {
	...
	if err != nil {
		r.logger.Error("Failed to marshal value for key "+key, zap.Error(err))
		return err
	}
	if err = r.redisClient.Set(ctx, key, data, time.Duration(ttl)*time.Second).Err(); err != nil {
		r.logger.Error("Failed to set to redis value for key "+key, zap.Error(err))
		return err
	}
	return nil
}
