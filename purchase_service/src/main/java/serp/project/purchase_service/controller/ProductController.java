...
public class ProductController {
    ...
    public ResponseEntity<GeneralResponse<PageResponse<ProductEntity>>> getProducts(
        ...
        log.info("[ProductController] Get products with page {}, size {}, sortBy {}, sortDirection {}, query {}, categoryId {}, statusId {}",
                page, size, sortBy, sortDirection, query, categoryId, statusId);
        Long tenantId = authUtils.getCurrentTenantId()
                .orElseThrow(() -> new AppException(AppErrorCode.UNAUTHORIZED));
        log.info("[ProductController] Current tenantId {}", tenantId);
        ...
        return ResponseEntity.ok(GeneralResponse.success("Successfully return products at page " + page, PageResponse.of(products)));
    }
}
