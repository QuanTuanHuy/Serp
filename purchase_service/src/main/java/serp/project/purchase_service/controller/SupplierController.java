...
public class SupplierController {
        ...
        @PostMapping("/create")
        public ResponseEntity<GeneralResponse<?>> createSupplier(@Valid @RequestBody SupplierCreationForm form) {
        ...
        public ResponseEntity<GeneralResponse<?>> updateSupplier(@Valid @RequestBody SupplierUpdateForm form,
                        @PathVariable("supplierId") String supplierId) {
                Long tenantId = authUtils.getCurrentTenantId()
                                .orElseThrow(() -> new AppException(AppErrorCode.UNAUTHORIZED));
                log.info("[SupplierController] Updating supplier {} with ID {} for tenantId {}", form.getName(),
                                supplierId,
                                tenantId);
        ...
        public ResponseEntity<GeneralResponse<SupplierEntity>> getDetailSupplier(
                        @PathVariable("supplierId") String supplierId) {
                Long tenantId = authUtils.getCurrentTenantId()
                                .orElseThrow(() -> new AppException(AppErrorCode.UNAUTHORIZED));
                log.info("[SupplierController] Getting detail supplier with ID {} for tenantId {}", supplierId,
                                tenantId);
                ...
                return ResponseEntity.ok(GeneralResponse.success("Successfully get supplier detail", supplier));
        }
