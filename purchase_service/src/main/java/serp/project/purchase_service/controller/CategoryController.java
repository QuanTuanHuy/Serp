...
public class CategoryController {
    ...
    public ResponseEntity<GeneralResponse<?>> updateCategory(@RequestBody CategoryForm form, @PathVariable("categoryId") String categoryId) {
        Long tenantId = authUtils.getCurrentTenantId()
                .orElseThrow(() -> new AppException(AppErrorCode.UNAUTHORIZED));
    ...
    public ResponseEntity<GeneralResponse<PageResponse<CategoryEntity>>> getCategories(
        ...
        Page<CategoryEntity> categories = categoryService.getCategories(
                ...
                sortBy,
                sortDirection);
        return ResponseEntity.ok(GeneralResponse.success("Successfully get list of category page " + page, PageResponse.of(categories)));
    }
