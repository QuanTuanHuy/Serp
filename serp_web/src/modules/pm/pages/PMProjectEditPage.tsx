...
export function PMProjectEditPage({ projectId }: PMProjectEditPageProps) {
  ...
  const { data: projectResponse, isLoading: isProjectLoading, error: projectError } =
  ...
  const mappedProject = useMemo(() => {
    ...
    const mapped: PMProjectDetail & { categoryName?: string } = {
      ...
      category: projectResponse.category ? String(projectResponse.category.id) : 'ALL',
      ...
      lead: {
        ...
        name: projectResponse.leadUserName || `User #${projectResponse.leadUserId}`,
      },
      visibility: 'ORGANIZATION',
      startDate: new Date(projectResponse.createdAt || Date.now()).toISOString().split('T')[0],
      targetDate: new Date(projectResponse.updatedAt || Date.now()).toISOString().split('T')[0],
    ...
    try {
      await updatePmProject({
        ...
        body: {
          ...
          categoryId: values.category === 'ALL' ? undefined : Number(values.category),
        },
      }).unwrap();
    ...
    return (
      <Card className="border border-border/70 bg-card/60 shadow-sm">
        ...
        <CardContent className="space-y-4">
          ...
          <Button type="button" onClick={() => router.push('/pm/projects')} className="rounded-xl">
            Back to projects
          </Button>
        </CardContent>
      </Card>
