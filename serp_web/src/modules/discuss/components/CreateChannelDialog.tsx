...
export const CreateChannelDialog: React.FC<CreateChannelDialogProps> = ({
  ...
  const [createDirectChannel, { isLoading: isCreatingDirect }] = useCreateDirectChannelMutation();
            ...
            <TabsContent value="channel" className="mt-0 space-y-4">
              <form onSubmit={handleCreateChannel} className="space-y-4">
                ...
                <div className='grid grid-cols-2 gap-3'>
                  ...
                  >
                    <Users className={cn('h-5 w-5', channelType === 'GROUP' ? 'text-violet-600' : 'text-slate-500')} />
                    <span className={cn('font-semibold', channelType === 'GROUP' ? 'text-violet-700 dark:text-violet-300' : 'text-slate-700')}>
                      Group
                    </span>
                  </button>
                  ...
                  >
                    <Hash className={cn('h-5 w-5', channelType === 'TOPIC' ? 'text-violet-600' : 'text-slate-500')} />
                    <span className={cn('font-semibold', channelType === 'TOPIC' ? 'text-violet-700 dark:text-violet-300' : 'text-slate-700')}>
                      Topic
                    </span>
                  </button>
                </div>
                ...
                <div className='space-y-2'>
                  <Label htmlFor='channel-name'>Channel Name <span className='text-rose-500'>*</span></Label>
                  <Input
                    id='channel-name'
                ...
                <div className='flex gap-3 pt-2'>
                  <Button type='button' variant='outline' onClick={resetAndClose} className='flex-1' disabled={isLoading}>
                    Cancel
                  </Button>
                  <Button 
                    type='submit' 
                  ...
                  >
                    {isLoading ? <Loader2 className='h-4 w-4 mr-2 animate-spin' /> : 'Create Channel'}
                  </Button>
                </div>
              </form>
            </TabsContent>
