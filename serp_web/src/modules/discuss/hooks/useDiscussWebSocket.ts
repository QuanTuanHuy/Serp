...
export const useDiscussWebSocket = (options: UseDiscussWebSocketOptions = {}) => {
  ...
  const wsUrl = process.env.NEXT_PUBLIC_WS_URL || 'ws://localhost:8080/ws/discuss';
  ...
  const handleChannelMessage = useCallback((payload: any) => {
  ...
  const sendTypingIndicator = useCallback((isTyping: boolean) => {
    const client = clientRef.current;
    if (!client || !client.connected || !channelId) {
      console.warn('[WebSocket] Cannot send typing indicator: not connected or no channel');
      return;
    }
  ...
  const markAsRead = useCallback((messageId: string) => {
    const client = clientRef.current;
    if (!client || !client.connected || !channelId) {
      console.warn('[WebSocket] Cannot mark as read: not connected or no channel');
      return;
    }
  ...
  const sendMessage = useCallback((content: string, parentId?: string) => {
    const client = clientRef.current;
    if (!client || !client.connected || !channelId) {
      console.warn('[WebSocket] Cannot send message: not connected or no channel');
      return;
    }
