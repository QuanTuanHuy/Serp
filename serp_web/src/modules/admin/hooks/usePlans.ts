...
export function usePlans() {
  ...
  const { data: plans = [], isLoading, error, refetch } =
    useGetSubscriptionPlansQuery();
  ...
  const setPlansViewMode = useCallback((mode: 'grid' | 'table') => {
    dispatch(setViewMode(mode));
  }, [dispatch]);

  const createPlan = useCallback(async (data: CreateUpdatePayload) => {
    try {
  ...
  const updatePlan = useCallback(async (id: number | string, data: Partial<SubscriptionPlan>) => {
    try {
  ...
  const toggleActive = useCallback(async (id: number | string, isActive: boolean) => {
    try {
      await updatePlanMutation({ id: String(id), data: { isActive: !isActive } }).unwrap();
      notification.success(`Plan ${isActive ? 'deactivated' : 'activated'} successfully`);
    } catch (err: any) {
      notification.error(getErrorMessage(err));
    }
  }, [updatePlanMutation, notification]);

  const deletePlan = useCallback(async (id: number | string) => {
    try {
  ...
  const submitPlan = useCallback(async (data: any) => {
    if (selectedPlan) {
