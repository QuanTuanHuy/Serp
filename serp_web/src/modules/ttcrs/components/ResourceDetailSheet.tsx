...
export function ResourceDetailSheet({
  ...
  // Mutations
  const [updateContainer, { isLoading: savingContainer }] =
    useUpdateDispatcherContainerMutation();
  const [deleteContainer, { isLoading: deletingContainer }] =
    useDeleteDispatcherContainerMutation();
  const [updateTruck, { isLoading: savingTruck }] =
    useUpdateDispatcherTruckMutation();
  const [deleteTruck, { isLoading: deletingTruck }] =
    useDeleteDispatcherTruckMutation();
  const [updateTrailer, { isLoading: savingTrailer }] =
    useUpdateDispatcherTrailerMutation();
  const [deleteTrailer, { isLoading: deletingTrailer }] =
    useDeleteDispatcherTrailerMutation();
  const [updateDriver, { isLoading: savingDriver }] =
    useUpdateDispatcherDriverMutation();
  ...
  const isSaving =
    savingContainer || savingTruck || savingTrailer || savingDriver;
  ...
  async function handleSave() {
    ...
    try {
        ...
        case 'CONTAINER':
          await updateContainer({
            ...
            body: {
              ...
              currentLocationCode: currentLocationCode || undefined,
            },
          }).unwrap();
          break;
        case 'TRUCK':
      ...
      toast.success(
        `${kind.charAt(0) + kind.slice(1).toLowerCase()} updated successfully`
      );
      onClose();
    } catch (err) {
  ...
  async function handleDelete() {
    ...
    try {
      switch (kind) {
        case 'CONTAINER':
          await deleteContainer(resource.id).unwrap();
          break;
        case 'TRUCK':
          await deleteTruck(resource.id).unwrap();
          break;
        case 'TRAILER':
          await deleteTrailer(resource.id).unwrap();
          break;
      }
            ...
            {!isDriver && (
              <div className='space-y-1.5'>
                ...
                <Select
                  ...
                  onValueChange={setCurrentLocationCode}
                >
                  <SelectTrigger className='w-full'>
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              <strong>{resource.identifier}</strong> will be permanently
