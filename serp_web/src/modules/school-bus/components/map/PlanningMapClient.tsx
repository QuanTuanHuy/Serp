  ...
  const markerContent = (
    <>
      ...
      {isVisible('pickup') &&
        pickupPoints.map((pp) => {
          if (
            typeof pp.latitude !== 'number' ||
            typeof pp.longitude !== 'number'
          )
            return null;
          return (
            <Marker
      ...
      {isVisible('pickup') &&
        sortedStops
          .filter(
            (s) =>
              s.stopPurpose !== 'START_TERMINAL' &&
              s.stopPurpose !== 'END_TERMINAL'
          )
          .map((stop, idx) => {
            const lat =
      ...
      {resolvedLinePositions.length >= 2 && (
        <Polyline
          ...
          color={
            selectedRoute?.routeDirection === 'RETURN' ? '#059669' : '#0284c7'
          }
          weight={4}
          opacity={0.85}
        />
